...
if ((isset($_POST['add_address'])) && ($_POST['add_address'] == 1)) {
    ...
    */
    ...
    if (!empty($user_id) && !empty($type) && !empty($name) && !empty($mobile) && !empty($address) && !empty($landmark) && (!empty($area_id) || !empty($area_name))  && (!empty($pincode_id) || !empty($pincode)) && (!empty($city_id) || !empty($city_name)) && !empty($state) && !empty($country)) {
        ...
        if ($res_insert) {
            $d_charges = $fn->get_data('area', 'id=' . $area_id, $columns = ['minimum_free_delivery_order_amount', 'delivery_charges']);
            $res = $db->getResult();
    ...
    */
    ...
    if (!empty($id) && !empty($user_id)) {
        ...
        if ($fn->is_address_exists($id)) {
            ...
            $update = $db->update('user_addresses', 'id=' . $id, $data);
            $updates = $db->getResult();

            if ($updates[0] == 1) {
                $d_charges = $fn->get_data('area', 'id=' . $area_id, $columns = ['minimum_free_delivery_order_amount', 'delivery_charges', 'name', 'pincode_id', 'city_id']);
            ...
            if ($type == 'checkout') {
                ...
                if (!empty($res)) {
                    ...
                    for ($i = 0; $i < count($res); $i++) {
                        if (isset($res[$i]['area_id']) && !empty($res[$i]['area_id'])) {
                            $area_data = $fn->get_data('area', 'id=' . $res[$i]['area_id'], $columns = ['minimum_free_delivery_order_amount', 'delivery_charges']);
                        } else {
                            $area_data = $fn->get_data('area', 'name= "' . trim($res[$i]['area'] . '"'), ['minimum_free_delivery_order_amount', 'delivery_charges']);
                        }

                        $res[$i]['minimum_free_delivery_order_amount'] = (!empty($area_data[0]['minimum_free_delivery_order_amount'])) ? $area_data[0]['minimum_free_delivery_order_amount'] : "0";
                        $res[$i]['delivery_charges'] = (!empty($area_data[0]['delivery_charges'])) ? $area_data[0]['delivery_charges'] : "0";
